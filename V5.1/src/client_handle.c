    ...
    while (1) {
        ...
        // GET (下载文件)
            ...
            if (resp.code == RESP_CONTINUE) {
                ...
                // 设置文件大小（预分配空间）
                if (ftruncate(file_fd, remote_file_size) < 0) {
                    printf("Failed to allocate file space: %s\n", strerror(errno));
                    close(file_fd);
                    close(data_fd);
                    continue;
                }
                ...
                while (total_received < remote_file_size) {
                    off_t remaining = remote_file_size - total_received;
                    ssize_t to_read = remaining > sizeof(buffer) ? sizeof(buffer) : remaining;
                    
                    ssize_t received = recv(data_fd, buffer, to_read, MSG_WAITALL);
                    if (received <= 0) {
                        printf("Failed to receive file data at offset %lld (remaining: %lld, to_read: %zd)\n", 
                               (long long)total_received, (long long)remaining, to_read);
                        if (received < 0) {
                            printf("Error: %s\n", strerror(errno));
                        }
                        break;
