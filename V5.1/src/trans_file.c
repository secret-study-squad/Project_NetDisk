...
int send_file(int conn_fd, const char *file_path) {
    ...
    // 循环使用sendfile零拷贝发送文件（处理大文件）
    while (total_sent < file_size) {
        off_t offset = total_sent;
        off_t remaining = file_size - total_sent;
        
        ssize_t sent = sendfile(conn_fd, file_fd, &offset, remaining);
        if (sent < 0) {
            LOG_ERROR("sendfile failed at offset %lld: %s", 
                     (long long)total_sent, strerror(errno));
            close(file_fd);
            return -1;
        }
        
        if (sent == 0) {
            LOG_ERROR("sendfile returned 0 at offset %lld (connection closed?)", 
        ...
        total_sent += sent;
        LOG_DEBUG("Sent %lld/%lld bytes (%.1f%%)", 
                 (long long)total_sent, (long long)file_size,
                 (double)total_sent * 100 / file_size);
    }
    
    LOG_INFO("File sent successfully: %s, size=%lld bytes", 
