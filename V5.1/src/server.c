...
time_wheel_t g_time_wheel;  // 全局时间轮
...
int main(int argc, char *argv[]){
    ...
    while(g_running){
        ...
        // 使用1秒超时，以便定期检查超时会话
        int nready = epoll_wait(epfd, lst, 10, 1000);  // 1000ms = 1秒
        ...
        if (current_time - last_timeout_check >= 1) {
            int timeout_fds[10];
            int timeout_count = time_wheel_check_timeout(&g_time_wheel, timeout_fds, 10);
            
            // 关闭超时的连接
            for (int i = 0; i < timeout_count; i++) {
                LOG_INFO("Closing timeout connection fd=%d", timeout_fds[i]);
                close(timeout_fds[i]);
                // 从epoll中移除
                epoll_ctl(epfd, EPOLL_CTL_DEL, timeout_fds[i], NULL);
            }
...
cleanup:
    ...
    // 3. 销毁时间轮
    LOG_INFO("Destroying time wheel...");
