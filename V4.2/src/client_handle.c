...
int recv_response(int conn_fd, response_header_t *resp_header, char *data, int max_len) {
    // 接收响应头部（包含token字段）
    if (recv(conn_fd, resp_header, sizeof(response_header_t), MSG_WAITALL) != sizeof(response_header_t)) {
    ...
    // 接收数据
    if (resp_header->data_len > 0 && data != NULL) {
...
int client_handle(int conn_fd, const char *server_ip, int data_port) {
    ...
    while (1) {
        ...
        if (strcmp(cmd, "register") == 0) {
            ...
            send_command(conn_fd, CMD_REGISTER, 0, data, strlen(data) + 1);
        ...
        // 登录
            ...
            send_command(conn_fd, CMD_LOGIN, 0, data, strlen(data) + 1);
        ...
        // PWD
            ...
            send_command(conn_fd, CMD_PWD, user_id, current_path, strlen(current_path) + 1);
        ...
        // CD
            ...
            send_command(conn_fd, CMD_CD, user_id, data, strlen(data) + 1);
        ...
        // LS
            ...
            send_command(conn_fd, CMD_LS, user_id, current_path, strlen(current_path) + 1);
        ...
        // MKDIR
            ...
            send_command(conn_fd, CMD_MKDIR, user_id, data, strlen(data) + 1);
        ...
        // RM
            ...
            send_command(conn_fd, CMD_RM, user_id, data, strlen(data) + 1);
        ...
        // PUT (上传文件)
            ...
            if (resp.code == RESP_SUCCESS) {
                ...
                // 需要上传文件，连接到数据端口
                printf("Uploading file via data connection...\n");
        ...
        // GET (下载文件)
            ...
            send_command(conn_fd, CMD_GET, user_id, data, strlen(data) + 1);
