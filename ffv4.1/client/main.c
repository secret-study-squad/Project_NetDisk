...
int main(int argc, char* argv[]){                                  
    ...
    int printUserFlag=1;
    while (!logged_in) {
        if(argc==1&&printUserFlag==1){
        printp("(输入 'login <user>' 登录 / 'register <user>' 注册)");
        //printf("\033[90m(输入 'login user' 登录 / 'register user' 注册)\033[0m\n");
        printf("\033[1;37mInput > \033[0m");
        fflush(stdout);
        printUserFlag=0;
        }
        if (!handling_cmd) {
        ...
        if (argc!=1||FD_ISSET(STDIN_FILENO, &rfds)) {
            printUserFlag=1;
            if(argc==2){
                cmd_type=CMD_LOGIN;
                strcpy(username,argv[1]);
                argc=1;
            }else if(argc==3&&(strcmp(argv[2],"R")||(strcmp(argv[2],"register")))){
            ...
            // 发送密码
            char password[256] = {0};
            getAndPrintPass(password,sizeof(password));
    ...
    while (1) {
        if(printUserFlag==1){
            const char* local_path = get_local_cur_path();
            printTitle(cur_user,cur_path,local_path);
             printUserFlag=0;
        }
        ...
        if (FD_ISSET(STDIN_FILENO, &rfds)) {
            ...
            if (cmd_type == CMD_REGISTER || cmd_type == CMD_LOGIN) {
                ...
                printw("Already logged in.\n");
                continue;
            }
            int is_local_cmd = (cmd_type == CMD_LPWD || 
                                cmd_type == CMD_LCD || 
                                cmd_type == CMD_LLS || 
                                cmd_type == CMD_LTREE);
            // 发送命令类型
            ...
            switch (cmd_type) {
                ...
                case CMD_PUTS:
                    ...
                    //发送远程路径预处理
                    char final_remote_path[256];
                    memset(final_remote_path, 0, sizeof(final_remote_path));
                    
                    int rp_len = strlen(path2);
                    // 获取本地文件名
                    char *base_name = strrchr(path1, '/');
                    base_name = base_name ? base_name + 1 : path1;

                    // 【修改】如果远程路径是 "." 或 "/"，直接发送文件名，让服务端默认放在当前目录
                    if (strcmp(path2, ".") == 0 || strcmp(path2, "/") == 0) {
                        strncpy(final_remote_path, base_name, sizeof(final_remote_path) - 1);
                    } 
                    // 如果远程路径以 "/" 结尾，拼接路径
                    else if (rp_len > 0 && path2[rp_len - 1] == '/') {
                        snprintf(final_remote_path, sizeof(final_remote_path), "%s%s", path2, base_name);
                    } 
                    // 其他情况（如指定了具体文件名或子路径），原样发送
                    else {
                        strncpy(final_remote_path, path2, sizeof(final_remote_path) - 1);
                    }
                    //处理好后发
                    send(client_fd, final_remote_path, sizeof(final_remote_path), MSG_NOSIGNAL);
                    cmd_puts(client_fd, path1, final_remote_path);
                    handling_cmd = 0;
            ...
            switch (cmd_type) {
                ...
                case CMD_TREE:    cmd_tree(client_fd);    break;
                ...
                case CMD_LCD: {
                    char path[1024];
                    //get_path(buf, path, NULL); // lcd 只有一个参数
                    get_path1(buf, path, sizeof(path));
                    cmd_lcd(client_fd, path);
                    break;
                }
                case CMD_LLS: {
                    char path[1024];
                    get_path1(buf, path, sizeof(path)); // lls 可选一个参数
                    cmd_lls(client_fd, path);
                    break;
                }
                case CMD_LTREE: {
                    char path[1024];
                    get_path1(buf, path, sizeof(path)); // ltree 可选一个参数
                    cmd_ltree(client_fd, path);
                    break;
                }
                default: break;
            }
